...
def set_ghost_cells(u, cartComm):
    ...
    # determine which ghost cells get what
    if up == -2:
        u[0,:] = u[2,:];       # u_{0,j}    = u_{2,j}
    else:
        cartComm.Isend(buf=u[1,:],dest=up)
        r = cartComm.Irecv(buf=u[0,:],source=up)
        r.Wait() #wait to receive u
    if down == -2:
        u[NxLocal+1,:] = u[NxLocal-1,:];    # u_{Nx+1,j} = u_{Nx-1,j}
    else:
        cartComm.Isend(buf=u[NxLocal,:],dest=down)
        r = cartComm.Irecv(buf=u[NxLocal+1,:],source=down)
        r.Wait()
    if left == -2:
        u[:,0] = u[:,2];       # u_{i,0}    = u_{i,2}
    else:
        bufIn = np.array(u[:,0]) # buffer
        cartComm.Isend(buf=np.array(u[:,1]),dest=left)
        r = cartComm.Irecv(buf=bufIn,source=left)
        r.Wait()
        u[:,0] = bufIn.T
    if right == -2:
        u[:,NyLocal+1] = u[:,NyLocal-1];    # u_{i,Ny+1} = u_{i,Ny-1}
    else:
        bufIn = np.array(u[:,NyLocal+1])
        cartComm.Isend(buf=np.array(u[:,NyLocal]),dest=right)
        r = cartComm.Irecv(buf=bufIn,source=right)
        r.Wait()
        u[:,NyLocal+1] = bufIn.T

if __name__ == '__main__':
    comm = MPI.COMM_WORLD
    size = comm.Get_size() # number of procs
    rank = comm.Get_rank() # rank of current proc
    #Px = 1
    Py = size/Px # assume size is a power of 2
    assert Px*Py == size # sanity check
    
    cartComm = comm.Create_cart((Px,Py)) # create a Cart comm
    ...
    # Coordinates of current process is the Cart comm
